...
bool detect_chess_move(const string& prev_image_path, const string& curr_image_path, const string& output_image_path){
    ...
    putText(output_image, "1", from_center, FONT_HERSHEY_SIMPLEX, 1.0, Scalar(0, 0, 255), 2);
    putText(output_image, "2", to_center, FONT_HERSHEY_SIMPLEX, 1.0, Scalar(0, 255, 0), 2); 
...
int main() {
    // Ví dụ sử dụng
    string prev_image_path = "/home/lephuc/robotic-chess-playing-arm/reference_image/previous .png";
    string curr_image_path = "/home/lephuc/robotic-chess-playing-arm/reference_image/current.png";
    string output_image_path = "/home/lephuc/robotic-chess-playing-arm/result.jpg";
    
    bool success = detect_chess_move(prev_image_path, curr_image_path, output_image_path);
    
    if (!success) {
        cerr << "Không thể xử lý ảnh!" << endl;
        return -1;
    }
